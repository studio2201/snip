//! doctor.rs — System & Environment Diagnostics for snip.
...
fn check_binary() -> DoctorCheck {
    let exe = env::current_exe().unwrap_or_else(|_| PathBuf::from("snip"));
    if !exe.exists() {
        return DoctorCheck {
            name: "binary_integrity".into(),
            status: "error".into(),
            detail: format!("Binary not found at {}", exe.display()),
            remediation: Some("Reinstall snip via install.sh".into()),
        };
    }
